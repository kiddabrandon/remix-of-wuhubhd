...
-- Grant admin role to the designated email on signup and on email confirmation
...
BEGIN
  IF NEW.email_confirmed_at IS NOT NULL
     AND lower(NEW.email) = 'ryanbradley639@gmail.com' THEN
...
-- Backfill: if the user already exists, grant them now
INSERT INTO public.user_roles (user_id, role)
SELECT id, 'admin'::app_role FROM auth.users
WHERE lower(email) = 'ryanbradley639@gmail.com'
